...
function buildSharedClosingInstructions(ident, health = null) {
  return [
    ...
    ``,
    `ESCALATING TO THE OWNER. If the work genuinely needs his judgment — real product, business, legal, financial or privacy consequences, an irreversible action, or a safety-critical system such as real-money trading — do not decide it yourself. Escalate, and stop.`,
    `Escalation REQUIRES a decision brief. An escalation that names a topic instead of a decision is refused and nothing is written to the board. Write the brief to a JSON file first, then pass its path:`,
    `  node ops-watcher/ahmad-escalate.mjs "${ident}" "<your reason, in Bahasa Indonesia>" <path/to/brief.json>`,
    `All five slots are required, in formal Indonesian, addressing him as "Anda": pertanyaan (must end with "?"), yang_sudah_ada (each entry a kutipan plus the sumber you actually read it from — a file, a commit, a link), pilihan (2-5, each with key, label and konsekuensi), rekomendasi (its pilihan must be one of those keys, plus alasan), kalau_didiamkan.`,
    `Go and read the sources before you write it. A brief that says "needs your input" without saying what the current state is, what the choices are, and what waiting costs is exactly the failure this gate exists to stop.`,
    `If it is refused, the refusal names the slots you left out. Fill them and send again — never resend the same brief.`,
    ``,
...
function buildStuckRecoveryPacket(it, health = null) {
  ...
  const opening = [
    ...
    `  (b) if resolving this genuinely requires the OWNER's judgment — it has real product, business, legal, financial, or privacy consequences, touches an irreversible action, or concerns a safety-critical system (e.g. real-money trading) — do NOT attempt a fix yourself; escalate instead, and stop.`,
    `      Escalation requires a decision brief — the full contract is in the ESCALATING TO THE OWNER section below, and ops-watcher/decision-brief.mjs has the exact shape.`,
