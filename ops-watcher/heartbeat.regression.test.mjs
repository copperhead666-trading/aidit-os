...
//
// Covers:
//   (H1) all 17 steps are attempted even if step 2 fails (the sweep must never
...
//   (H12) the STEPS pipeline has 17 entries and the LAST one is the
...
// =====================================================================
// H1: all 17 steps attempted even if step 2 (test-runner) fails
// =====================================================================
async function testAllStepsAttemptedOnStep2Fail() {
  const name = "H1 all 17 steps attempted even when step 2 (test-runner) fails";
  ...
  try {
    ...
    assert.equal(r.total, STEPS.length, "exactly 17 steps in the pipeline");
    assert.equal(r.results.length, STEPS.length, "all 17 steps produced a result");
...
async function testSummaryLinesProduced() {
  ...
  try {
    ...
    // One START line, one per-step line (17), one final DONE line.
    ...
    assert.equal(stepLines.length, STEPS.length, "one summary line per step (17)");
    ...
    for (const sl of stepLines) {
      assert.match(sl, /\[(watcher|test-runner|review-runner|telegram-notify|telegram-listener|cockpit-status|ahmad-dispatch|steward|steward-sjs|steward-caveman|escalation-sec|gbrain-curator|audit-clerk|self-repair|graphify-refresh|reconcile|directive-runner)\]/, "step line has step name");
      assert.match(sl, /exit=/, "step line has exit code");
      assert.match(sl, /(OK|FAIL)/, "step line has OK/FAIL");
    }
    // Final summary reports succeeded=17/17 failed=0/17.
    const done = doneLines[0];
    assert.match(done, new RegExp(`succeeded=${STEPS.length}/${STEPS.length}`), "final summary succeeded=17/17");
    assert.match(done, new RegExp(`failed=0/${STEPS.length}`), "final summary failed=0/17");
...
async function testMissingScriptDoesNotCrash() {
  ...
  try {
    const r = await runHeartbeatOnce({ runStep: makeFakeRunStep(table), log, now: () => 1700000000000, shouldRunTelegramListenerStep: alwaysRunFallback, appendStepLog: noopAppendStepLog });
    assert.equal(r.results.length, STEPS.length, "still 17 results — sweep ran to completion");
    ...
    const after = r.results.filter((x) => ["telegram-notify", "telegram-listener", "cockpit-status", "ahmad-dispatch", "steward", "steward-sjs", "steward-caveman", "escalation-sec", "gbrain-curator", "audit-clerk", "self-repair", "graphify-refresh", "reconcile", "directive-runner"].includes(x.name));
...
async function testArgvDrift() {
  ...
  try {
    ...
    assert.deepEqual(seen[15], ["ops-watcher/reconcile.mjs", "--once"]);
    assert.deepEqual(seen[16], ["ops-watcher/directive-runner.mjs", "--once"]);
    ok(name);
  } catch (err) { bad(name, err); }
}
...
async function testTelegramListenerSkippedWhenDaemonHealthy() {
  ...
  try {
    ...
    assert.equal(r.total, STEPS.length, "still 17 steps reported");
    assert.equal(r.results.length, STEPS.length, "still 17 results produced");
    assert.deepEqual(r.results.map((x) => x.name), STEP_NAMES);
...
async function testSelfRepairIsFourteenthStep() {
  const name = "H11 STEPS has 17 entries; 14th is self-repair; last is directive-runner with mandated argv";
  const seen = [];
  ...
  try {
    ...
    assert.equal(STEPS.length, 17, "expected STEPS fixture has 17 entries");
    assert.equal(r.total, STEPS.length, "pipeline total is 17");
    assert.equal(seen.length, STEPS.length, "17 steps were dispatched");
...
async function testDirectiveRunnerIsFifteenthStep() {
  const name = "H12 STEPS has 17 entries; last is directive-runner with argv [\"ops-watcher/directive-runner.mjs\", \"--once\"]";
  const seen = [];
  const runStep = async (argv) => { seen.push(argv.slice()); return { code: 0, stdout: "", stderr: "", error: null }; };
  try {
    const r = await runHeartbeatOnce({ runStep, log: () => {}, now: () => 1700000000000, shouldRunTelegramListenerStep: alwaysRunFallback, appendStepLog: noopAppendStepLog });
...
async function testHeartbeatLockFreeRunsAndReleases() {
  const name = "L1 lock-free heartbeat runs all 17 steps and releases after step-log append";
  const events = [];
  const seen = [];
  ...
  try {
    ...
    assert.equal(r.results.length, STEPS.length, "all 17 steps produced results");
    assert.equal(seen.length, STEPS.length, "all 17 steps ran");
...
async function testHeartbeatDeadLockIsReclaimedBySharedPrimitive() {
  const name = "L3 dead-held heartbeat lock is reclaimed by shared primitive and all 17 steps run";
  ...
  try {
    ...
    assert.equal(r.results.length, STEPS.length, "all 17 steps produced results after stale lock reclaim");
    assert.equal(seen.length, STEPS.length, "all 17 steps ran after stale lock reclaim");
...
async function testNotPausedRunsAllFifteenSteps() {
  const name = "P4 not paused heartbeat still dispatches all 17 steps";
  const seen = [];
  const runStep = async (argv) => { seen.push(argv.slice()); return makeDefaultSuccessfulRunStep()(argv); };
  try {
    const r = await runHeartbeatOnce({
    ...
    assert.equal(seen.length, STEPS.length, "all 17 steps dispatched");
