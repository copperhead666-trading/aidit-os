...
|---|---|---|
| 01 Orchestrator | Claude Code headless, langganan | Claude Code headless **akun mesin** (`pusatberasmurah`, `CLAUDE_CONFIG_DIR=D:\aidit-claude-machine`, login Kamis): Conductor keputusan + review Engineering/QA. Sesi manusia/orkestrator = **akun fresh** (`adityainofficial`, `~/.claude`), Sonnet, ≤ 150 turn |
...
- Akun Claude (revisi 09:05 WIB, keadaan nyata): `~/.claude` = akun fresh `adityainofficial` =
  **sesi orkestrator/Bapak** (terminal biasa). Akun `pusatberasmurah` (99 % weekly, reset Kamis
  2026-09-17 06:00 WIB) = **mesin** (Conductor, QA, review) lewat `CLAUDE_CONFIG_DIR=D:\aidit-claude-machine`
  di PM2 — login Kamis pagi dengan `ops/claude-machine.cmd`. Kedua login hidup bersamaan; `claude auth
  status` per folder menunjukkan email. Sampai Kamis pool Claude mesin = `disabled` (fallback GLM).
...
|---|---|---|---|
| Claude mesin (`pusatberasmurah`, reset Kamis 06:00) | Conductor keputusan (sonnet ≤ 5/hari, opus 0), QA review diff (≤ 8 turn), Engineering review | ≤ 15 panggilan headless | `CLAUDE_CONFIG_DIR=D:\aidit-claude-machine`; `--setting-sources ""` |
| Claude Bapak/orkestrator (`adityainofficial`, fresh) | sesi manusia/orkestrator (Sonnet) | ≤ 150 turn/sesi, wakeup 60 menit | `~/.claude` default (terminal biasa) |
...
5. Akun Claude: `ops/claude-machine.cmd` (set `CLAUDE_CONFIG_DIR=D:\aidit-claude-machine` lalu `claude auth login`);
   Bapak login akun mesin (`pusatberasmurah`) di situ Kamis ≥ 06:00 WIB. Sesi orkestrator memakai `~/.claude` (fresh).
