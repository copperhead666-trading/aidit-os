...
### Dashboard extension (FounderOS Cockpit)

The ramp above is a marketing ramp: it bottoms out at 14px because nothing on
zapier.com is denser than a pricing table. An operations deck read on a phone
every morning needs two steps below it, and needs them named — eleven ad-hoc
sizes between 10px and 13.5px is noise, not a system.

| Token | Size | Weight | Use |
|---|---|---|---|
| `{typography.ui-title}` | 24px | 600 | Page title. Reuses `display-sub-sm`. |
| `{typography.ui-lead}` | 20px | 600 | Section lead, the one decision that needs the owner. |
| `{typography.ui-body}` | 16px | 400 | Card body — reuses `body-sm`. |
| `{typography.ui-default}` | 14px | 400 | Default interface text. Reuses `caption`. |
| `{typography.ui-dense}` | 13px | 400 | List rows, secondary detail. |
| `{typography.ui-meta}` | 12px | 400/500 | Identifiers, timestamps, counts, status chips. |

**Nothing renders below 12px.** Small text on a phone held at arm's length in
morning light is not a density win, it is an unread interface.
