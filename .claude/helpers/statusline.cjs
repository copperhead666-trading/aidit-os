...
function resolveCliBinCandidates() {
  ...
  try {
    ...
    try {
      ...
      // Windows: npm's default global prefix (%APPDATA%\npm) is never in
      // npm_config_prefix/PREFIX unless a user customized it, and this script
      // isn't spawned via `npm run` so npm never injects that env var either
      // — so without this, the whole Windows-default-prefix case silently
      // resolved zero candidates. node_modules sits directly under the
      // prefix on Windows (no nested lib/), unlike the POSIX layout above.
      if (process.env.APPDATA) globalModuleDirs.push(path.join(process.env.APPDATA, 'npm', 'node_modules'));
      for (const gm of globalModuleDirs) {
        candidates.push(
          path.join(gm, 'ruflo', 'bin', 'cli.js'),
          path.join(gm, '@claude-flow', 'cli', 'bin', 'cli.js'),
          // The published `ruflo` package has no bin/cli.js of its own (only
          // bin/ruflo.js, a thin wrapper) and declares @claude-flow/cli as a
          // real dependency rather than re-exporting it — so npm nests it
          // under ruflo's own node_modules instead of installing it as a
          // global sibling package. Without this candidate, every render on
          // a plain `npm install -g ruflo` falls through to the npx branch.
          path.join(gm, 'ruflo', 'node_modules', '@claude-flow', 'cli', 'bin', 'cli.js'),
        );
      }
    } catch { /* ignore */ }
  } catch { /* ignore */ }
